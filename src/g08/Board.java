...
public class Board implements Observer{
    ...
    public static int zobrist[][][]=new int[19][19][3];//zobrist
    static{
        Random r=new Random();
        for(int i=0;i<19;++i)
        {
    ...
    public int hashcode()//获取当前棋盘的zobrist哈希值
    {
        ...
        for(int i=0;i<19;++i)
        {
            ...
            {
                int k=0;
                if(this.get(i,j)== PieceColor.EMPTY) k=0;
                else if(this.get(i,j)==PieceColor.BLACK) k=1;
                else k=2;
                hash^=zobrist[i][j][k];
            }
        }
    ...
    public void makePoint(Point p, PieceColor c)
    {
        this._board[p.x+p.y*19]=c;
    }
    public void unMakePoint(Point p)
    {
        this._board[p.x+p.y*19]= EMPTY;
    }
    ...
    private final PieceColor[] _board = new PieceColor[361];//19*19
    ...
    public PieceColor get(int c, int r) {
        assert Move.validSquare(s.charAt(c), s.charAt(r));

        return this._board[Move.index(s.charAt(c), s.charAt(r))];
    }
    ...
    public void makeMove(int c0, int r0, int c1, int r1) {
        this.makeMove(new Move(s.charAt(c0), s.charAt(r0), s.charAt(c1), s.charAt(r1)));
    }
    ...
    public void unMakeMove(Move mov) {
        this._whoseMove = this._whoseMove.opposite();
        this.moveList.remove(mov);
        this.set(mov.col0(), mov.row0(), PieceColor.EMPTY);
        this.set(mov.col1(), mov.row1(), PieceColor.EMPTY);
    }
